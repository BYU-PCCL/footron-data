...
/*
 * Red-Black Tree — phone panel.
 * Choose the order the 26 keys arrive in and race the two trees; the live
 * heights come back from the wall as { order, orders[], n, plain, redblack, best }.
 */
const Panel = () => {
  const wall = useWall();
  const p = wall.panel || {};
  const orders = (p.orders || []).map((o) => ({ id: o.id, title: o.label, note: o.note }));

  return (
    <div css={pageStyle}>
      <p css={leadStyle}>
        Both trees get the same keys in the same order. A plain tree just grows; the red-black tree
        recolours and rotates to stay short. Pick an order and race them.
      </p>

      <div css={cardStyle}>
        <div css={css`display: flex; justify-content: space-around; text-align: center;`}>
          <div>
            <div css={css`font: 700 34px "Roboto Mono", Menlo, monospace; color: #b4535d;`}>{p.plain ?? "–"}</div>
            <div css={hintStyle}>plain tree height</div>
          </div>
          <div>
            <div css={css`font: 700 34px "Roboto Mono", Menlo, monospace; color: #2d7d6f;`}>{p.redblack ?? "–"}</div>
            <div css={hintStyle}>red-black height</div>
          </div>
        </div>
        <p css={css`${hintStyle}; text-align: center; margin-top: 8px;`}>
          {p.n ? `${p.n} keys in · the best any tree could do is ${p.best}` : "waiting for keys…"}
        </p>
      </div>

      {section("Race them on")}
      {orders.length ? choices(orders, p.order, (id) => wall.input("order", id)) : <p css={hintStyle}>Connecting to the wall…</p>}

      <Button css={bigButton} variant="outlined" color="primary" onClick={() => wall.action("one")}>
        Add one more random key
      </Button>
