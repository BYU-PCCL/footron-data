...
/* The path alone, with no beginPath, so callers can lay down two of these and
   clip on the difference between them. */
...
/* A filled box with an optional border, an optional glow, and an optional hard
   drop shadow behind it. This is the cell, the slot, the node and the card:
   close to everything on screen is one of these. */
function box(ctx, x, y, w, h, o){
  o = o || {};
  var r = o.r === undefined ? 0 : o.r;
  /* A solid offset copy rather than a blurred canvas shadow, so it reads as the
     flat `3px 3px 0` drop shadow it is meant to be. Clipped to the part that
     falls outside the box, since the fills here are translucent and would
     otherwise let the shadow show through the face of the box. */
  if (o.shadow){
    var dx = o.shadowDx === undefined ? 3 : o.shadowDx;
    var dy = o.shadowDy === undefined ? 3 : o.shadowDy;
    ctx.save();
    ctx.beginPath();
 ...
 */
var T = {
  ink:    "#0c050b",   /* the canvas ground                         */
  panel:  "#2c1425",   /* an empty cell, slot or node               */
  panel2: "#381b2e",   /* an empty cell that is being looked at     */
  line:   "#4e2848",   /* every border and rule                     */
  grid:   "#381c31",   /* the faintest structural line              */
  text:   "#fbe9f8",   /* a value, a key, anything being read       */
  muted:  "#c28eb7",   /* a label                                   */
  dim:    "#8c5b88",   /* an index, a tick, a disabled thing        */
...
global.D = {
  ...
  roundRect: roundRect, roundRectPath: roundRectPath, box: box, text: text, arrow: arrow, arcTo: arcTo,
