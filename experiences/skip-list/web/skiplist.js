  ...
  draw: function(ctx){
    ...
    for (lvl = 0; lvl < this.maxH; lvl++){
      D.box(ctx, X0, levelY(lvl), NODE_W, NODE_H, {
        r: 0, fill: D.fade(T.panel, 0.8), shadow: T.line
      });
    ...
    for (i = 0; i < this.nodes.length; i++){
      ...
      for (lvl = 0; lvl < height; lvl++){
        ...
        var fill = T.panel, edge = T.line, col = T.text, glow = null;
        if (state === "found"){ fill = D.fade(T.good, 0.18); edge = T.good; glow = D.fade(T.good, 0.55); }
        else if (state === "compare"){ fill = D.fade(ACCENT, 0.16); edge = ACCENT; glow = D.fade(ACCENT, 0.45); }
        else if (state === "reject"){ fill = D.fade(T.bad, 0.13); edge = D.fade(T.bad, 0.8); col = T.text; }
        else if (lvl > 0){ fill = D.fade(T.panel, 0.85); edge = D.fade(ACCENT, 0.32); }

        ctx.save();
        ctx.globalAlpha = (lvl === 0 ? n.vis : part);
        D.box(ctx, colX(i), ny + rise, NODE_W, NODE_H, {
          r: 0, fill: fill, shadow: edge, glow: glow, glowSize: 14
        });
  ...
  drawTally: function(ctx){
    ...
    for (var i = 0; i < pairs.length; i++){
      ...
      D.box(ctx, p.x, y, w, h, {
        ...
        shadow: p.on ? D.fade(p.color, 0.45) : T.line
      });
