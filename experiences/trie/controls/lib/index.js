...
/*
 * Trie: How Autocomplete Works — phone panel.
 * Add a word the wall offers (so you see which letters it shares), then pick a
 * prefix and the wall autocompletes it. The wall sends
 * { words, offer[], prefixes[{p, n}], prefix, suggestions[] } as `panel`.
 */
const Panel = () => {
  const wall = useWall();
  const p = wall.panel || {};
  const offer = (p.offer || []).map((w) => ({ id: w, label: w }));
  const prefixes = (p.prefixes || []).map((x) => ({ id: x.p, label: `${x.p}… ${x.n}` }));

  return (
    <div css={pageStyle}>
      <p css={leadStyle}>
        Words that start the same way share the same branch. Add a word and watch which letters it gets for
        free — then ask for completions, the way a phone keyboard does.
      </p>

      {section("Add a word")}
      {offer.length ? chips(offer, null, (w) => wall.input("word", w)) : <p css={hintStyle}>Connecting to the wall…</p>}
      <Button css={bigButton} variant="outlined" color="primary" onClick={() => wall.action("fill")}>
        Add ten words at once
      </Button>

      {section("Autocomplete")}
      <p css={hintStyle}>Pick how a word starts. The number is how many stored words start that way.</p>
      {prefixes.length ? chips(prefixes, p.prefix, (x) => wall.input("prefix", x)) : null}

      {p.prefix ? (
        <div css={cardStyle}>
          <div css={css`font-weight: 700; margin-bottom: 6px;`}>The wall suggests, for “{p.prefix}…”</div>
          <div css={css`font: 16px "Roboto Mono", Menlo, monospace; line-height: 1.6;`}>{(p.suggestions || []).join(" · ")}</div>
        </div>
      ) : null}

      <p css={hintStyle}>{p.words ? `${p.words} words stored.` : ""}</p>
      {footer(wall)}
    </div>
  );
};
