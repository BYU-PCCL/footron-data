...
/*
 * Editing Together, Offline — phone panel.
 * Pick one of the wall's examples, or make the edits yourself: each laptop
 * types or deletes while offline, then sync them and watch both copies match.
 * The wall sends { example, examples[], state, a, b, playing } as `panel`.
 */
const STATE_LABEL = { offline: "Offline — each laptop edits alone", syncing: "Syncing…", identical: "Both copies match ✓", online: "Online" };
const STATE_COLOUR = { offline: "#b4535d", syncing: "#9a6a12", identical: "#2d7d6f", online: MUTED };

const Panel = () => {
  const wall = useWall();
  const p = wall.panel || {};
  const examples = (p.examples || []).map((e) => ({ id: e.id, title: e.title, note: e.blurb }));
  const st = p.state || "online";

  return (
    <div css={pageStyle}>
      <p css={leadStyle}>
        Two laptops edit one document while the Wi-Fi is down. When they reconnect, both copies must end up
        exactly the same — with no server deciding.
      </p>

      <div css={cardStyle}>
        <div css={css`font-weight: 700; color: ${STATE_COLOUR[st] || MUTED}; margin-bottom: 8px;`}>{STATE_LABEL[st] || st}</div>
        <div css={css`font: 16px "Roboto Mono", Menlo, monospace; line-height: 1.6;`}>
          <div><span css={css`color: #2d7d6f; font-weight: 700;`}>A</span> “{p.a || ""}”</div>
          <div><span css={css`color: #6a51a3; font-weight: 700;`}>B</span> “{p.b || ""}”</div>
        </div>
      </div>

      {section("Watch an example")}
      {examples.length ? choices(examples, p.example, (id) => wall.input("example", id)) : <p css={hintStyle}>Connecting to the wall…</p>}

      {section("Make your own edits")}
      <p css={hintStyle}>Editing takes that laptop offline. Make a change on each, then sync.</p>
      <div css={gridStyle}>
        <Button css={bigButton} variant="outlined" color="primary" onClick={() => wall.action("typeA")}>Laptop A types</Button>
        <Button css={bigButton} variant="outlined" color="primary" onClick={() => wall.action("typeB")}>Laptop B types</Button>
        <Button css={bigButton} variant="outlined" color="primary" onClick={() => wall.action("delA")}>Laptop A deletes</Button>
        <Button css={bigButton} variant="outlined" color="primary" onClick={() => wall.action("delB")}>Laptop B deletes</Button>
      </div>
      <Button css={bigButton} variant="contained" color="primary" onClick={() => wall.action("sync")}>
        Reconnect and sync
      </Button>
