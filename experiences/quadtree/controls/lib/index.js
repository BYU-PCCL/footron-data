...
/*
 * Quadtree — phone panel.
 * Drag the search circle over the flocks (the dark pad mirrors the wall), set
 * its size, and change how crowded a square may get before it splits. The
 * wall sends { aspect, flocks[[x, y, n]], q, radius, capacity, found, looked, total }.
 */
const Panel = () => {
  const wall = useWall();
  const p = wall.panel || {};
  ...
  const aspect = p.aspect || 3;

  useEffect(() => {
    const c = ref.current;
    if (!c) return;
    const { g, w, h } = fitCanvas(c);
    const colours = [WALL.teal, WALL.violet, "#78b8d8", "#a2c27c", "#c792b8"];
    (p.flocks || []).forEach((f, i) => {
      const r = 6 + Math.sqrt(f[2]) * 1.4;
      const grad = g.createRadialGradient(f[0] * w, f[1] * h, 0, f[0] * w, f[1] * h, r);
      grad.addColorStop(0, colours[i % colours.length]);
      grad.addColorStop(1, "rgba(0,0,0,0)");
      g.fillStyle = grad;
      g.fillRect(f[0] * w - r, f[1] * h - r, r * 2, r * 2);
    });
    const q = drag || (p.q ? { x: p.q[0], y: p.q[1] } : null);
    if (q) {
      const rr = (radius ?? p.radius ?? 0.14) * h;
      g.beginPath();
      g.arc(q.x * w, q.y * h, rr, 0, Math.PI * 2);
      g.fillStyle = "rgba(226,178,95,0.12)";
      g.fill();
  ...
  const move = (e) => {
    if (!drag && e.type === "pointermove") return;
    const pt = pointAt(e, ref.current);
    setDrag(pt);
    wall.live("move", pt);
  };
  ...
  return (
    <div css={pageStyle}>
      <p css={leadStyle}>
        Drag the circle onto a flock. The wall only opens the squares the circle touches — every other
        point is never even looked at.
      </p>

      <canvas
        ref={ref}
        css={css`${padStyle}; aspect-ratio: ${aspect};`}
        onPointerDown={(e) => { e.currentTarget.setPointerCapture(e.pointerId); const pt = pointAt(e, ref.current); setDrag(pt); wall.input("move", pt); }}
        onPointerMove={move}
        onPointerUp={() => setDrag(null)}
        onPointerCancel={() => setDrag(null)}
      />
      <p css={readoutStyle}>
        {!p.total ? "connecting to the wall…"
          : p.found ? `found ${p.found} inside · looked at ${p.looked} of ${p.total} points`
          : `nothing here · checked ${p.looked} of ${p.total} points to be sure`}
      </p>

      {slider("Circle size", radius ?? p.radius ?? 0.14, 0.05, 0.3, 0.01, (v) => `${Math.round(v * 100)}`,
        (v) => { setRadius(v); wall.live("radius", v); },
        (v) => { setRadius(null); wall.input("radius", v); })}
      {slider("A square splits once it holds more than", cap ?? p.capacity ?? 4, 1, 12, 1, (v) => `${v} point${v === 1 ? "" : "s"}`,
        (v) => setCap(v),
        (v) => { setCap(null); wall.input("capacity", v); })}

      <div css={gridStyle}>
        <Button css={bigButton} variant="outlined" color="primary" onClick={() => wall.action("burst")}>Add a flock</Button>
        <Button css={bigButton} variant="outlined" color="primary" onClick={() => wall.action("scatter")}>Scatter everything</Button>
      </div>
